...
/* ---- RatTrainer implementation ---- */

RatTrainer::RatTrainer()
  : _network(),
    _optimizer( nullptr ),
...
ActionResult RatTrainer::get_window_and_intersend( const Memory & memory, int current_window )
{
...
void RatTrainer::store_episode( double utility, const vector<ObsAction> & observations )
{
...
double collect_experience( RatTrainer & trainer,
                           ...
                           const unsigned int tick_count )
{
  ...
  for ( auto & config : configs ) {
    ...
    Network< SenderGang<NeuralRat, TimeSwitchedSender<NeuralRat>>,
      ...
      network( NeuralRat( trainer ), run_prng, config );
...
int main( int argc, char *argv[] )
{
  ...
  while ( 1 ) {
    double score = collect_experience( trainer, prng_seed, configs, tick_count );
